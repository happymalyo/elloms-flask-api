...
async def create_conversation(
    db: AsyncSession, user_id: int, conversation: ConversationCreate
) -> Conversation:
...
async def create_crew_job(
    db: AsyncSession, user_id: int, job_data: CrewJobCreate
) -> CrewJob:
...
async def update_crew_job(
    db: AsyncSession, job_id: str, job_update: CrewJobUpdate
) -> Optional[CrewJob]:
...
async def get_crew_job(
    ...
    result = await db.execute(
        select(CrewJob).where(CrewJob.job_id == job_id, CrewJob.user_id == user_id)
    )
    return result.scalar_one_or_none()


async def get_user_crew_jobs(
    db: AsyncSession, user_id: int, skip: int = 0, limit: int = 50
) -> List[CrewJob]:
    result = await db.execute(
        select(CrewJob)
